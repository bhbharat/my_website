...
# Build the Docker image with provided Artifactory credentials and tag it
docker build --build-arg ARTIFACTORY_USERNAME=3477482 --build-arg ARTIFACTORY_PASSWORD= -t registry.web.boeing.com/bharat.bhushan/dsrm_xml/indexed_srm:1 .

# Push the Docker image to the specified registry
docker push registry.web.boeing.com/bharat.bhushan/dsrm_xml/indexed_srm:1

# URL to request OAuth token for OpenShift
https://oauth-openshift.apps.kcs-prod-clt.k8s.boeing.com/oauth/token/request 

# Import the image into OpenShift
oc import-image indexed-srm:1

# Restart the rollout if elasticsearch not working
http://es-boeing-ai.apps.kcs-prod-clt.k8s.boeing.com/
http://upload-boeing-ai.apps.kcs-prod-clt.k8s.boeing.com/upload


# Delete existing resources (imagestream, deployment, deployment config, service, route, persistent volume claim) labeled with app=dsrm-test
oc delete imagestream,deployment,dc,svc,route,pvc -l app=indexed-srm

# Create a new application in OpenShift using the specified image and name it dsrm-test
# This code was generated by Generative AI
oc new-app --image=registry.web.boeing.com/bharat.bhushan/dsrm_xml/indexed_srm:1 --name indexed-srm && oc expose service indexed-srm --port 8501 --name indexed-srm && oc expose service indexed-srm --port 9200 --name es && oc expose service indexed-srm --port 5000 --name upload && oc set resources deployment/indexed-srm --limits=cpu=2,memory=3Gi --requests=cpu=2,memory=2Gi



# Edit the route for dsrm-test
oc edit route.route.openshift.io/indexed-srm

# Specify the route configuration
spec:
  host: time-cpuser1-test.apps.kcstd-clt.cloud.boeing.com
  port:
    targetPort: 8080-tcp
...
# Add a persistent volume claim to the deployment and mount it at /feedbacks
# choose this pvc - ocs-storageclass-cephfs. not block pvs
oc set volume deployment.apps/indexed-srm --add -t pvc --claim-name=elasticsearch-data --mount-path=/usr/share/elasticsearch/data
oc set volume deployment.apps/indexed-srm --add -t pvc --claim-name=pdf-files --mount-path=/data

# Check the disk space usage of the mounted volume
